...
class IndependentUndirectedEdgeInteractionBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.three_contact_gates:
            ...
            for head in (
                self.base_contact_head,
                self.e_contact_head,
                self.mu_contact_head,
            ):
                _zero_last(head)
    ...
    def forward(
        ...
        if self.capture_edges:
            ...
            self.last_mu_pair_event_gate = (
                None if mu_pair_event_gate is None else mu_pair_event_gate.detach()
            )
        ...
        self.last_base_contact_gate = (
            edge_gate if self.three_contact_gates else None
        )
    ...
    def forward(
        ...
        hidden_fp32, logits = group(
            ...
            previous_object_state=(
                self._continuous_state if self.continuous_object_state else None
            ),
            previous_route_indices=(
