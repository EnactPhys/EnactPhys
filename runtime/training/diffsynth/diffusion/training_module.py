...
class DiffusionTrainingModule(torch.nn.Module):
    ...
    def resume_from_checkpoint(self, path, remove_prefix_in_ckpt):
        ...
        loaded_trainable = expected_trainable & set(state_dict)
        missing_trainable = sorted(expected_trainable - loaded_trainable)
        if missing_trainable:
            raise ValueError(
                "Cannot load weight-only checkpoint with missing trainable tensors: "
