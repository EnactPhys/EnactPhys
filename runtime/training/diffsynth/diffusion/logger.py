import csv, errno, io, json, os, time, torch
...
class _ResilientAppendLogger:
    """Keep auxiliary scalar logging from killing a healthy distributed job.

    PFS can transiently return ENOSPC even when training tensors and optimizer
    state are healthy. Scalar files are evidence, not part of the optimizer
    transition, so retain a bounded in-memory backlog and retry on every log
    call. All other I/O failures remain fatal.
    """

    def __init__(self, path, max_pending_records=4096):
        self.path = path
    ...
    def _append(self, payload):
        ...
        fd = None
        try:
            fd = os.open(self.path, os.O_APPEND | os.O_CREAT | os.O_WRONLY, 0o666)
            while self._pending:
                ...
                if written == len(chunk):
                    ...
                    raise OSError(errno.ENOSPC, "zero-byte append to scalar log")
...
class JSONLLogger(_ResilientAppendLogger):
    """Dependency-free, append-only scalar log that survives full resume."""

    def __init__(self, log_dir):
        os.makedirs(log_dir, exist_ok=True)
        super().__init__(os.path.join(log_dir, "loss.jsonl"))

    def log(self, key, value, step):
        scalar = value.detach().float().cpu().item() if torch.is_tensor(value) else float(value)
        self._append(json.dumps({"step": int(step), key: scalar, "time": time.time()}) + "\n")

    def close(self):
        pass


class CSVLogger(_ResilientAppendLogger):
    """Append-only scalar table for direct train/validation comparison."""

    def __init__(self, log_dir):
        os.makedirs(log_dir, exist_ok=True)
        super().__init__(os.path.join(log_dir, "loss.csv"))
        if not os.path.exists(self.path):
            output = io.StringIO(newline="")
            csv.writer(output).writerow(["step", "metric", "value", "time"])
            self._append(output.getvalue())

    def log(self, key, value, step):
        scalar = value.detach().float().cpu().item() if torch.is_tensor(value) else float(value)
        output = io.StringIO(newline="")
        csv.writer(output).writerow([int(step), key, scalar, time.time()])
        self._append(output.getvalue())
...
class WandbLogger:
    def __init__(self, project_name="DiffSynth-Studio", log_dir=None):
        ...
        self.wandb = wandb
        kwargs = {"project": project_name, "dir": log_dir}
        optional = {
        ...
        print(
            f"Wandb is enabled. Project: {project_name}; "
            f"run_id={getattr(self.run, 'id', None)}; name={getattr(self.run, 'name', None)}"
        )
...
class ModelLogger:
    ...
    def init_loggers(self):
        self.loggers.append(JSONLLogger(os.path.join(self.output_path, "scalar_log")))
        self.loggers.append(CSVLogger(os.path.join(self.output_path, "scalar_log")))
        ...
        if accelerator.is_main_process:
            ...
            if loss is not None:
                for logger in self.loggers:
                    logger.log("train_loss", loss, self.num_steps)
        if save_steps is not None and self.num_steps % save_steps == 0:
            self.save_model(accelerator, model, f"step-{self.num_steps}.safetensors")

    def log_metric(self, accelerator: Accelerator, key: str, value, step: int) -> None:
