...
def add_dataset_base_config(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--compact_data_process_cache",
        default=False,
        action="store_true",
        help="For data_process only, omit decoded input_video/input_image from the saved latent cache.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Frozen validation metadata evaluated at step 0 and fixed intervals.",
    )
    parser.add_argument(
        "--validation_steps",
        type=int,
        default=None,
        help="Optimizer-step interval for deterministic full validation.",
    )
    parser.add_argument(
        "--validation_seed",
        type=int,
        default=424242,
        help="Fixed per-rank RNG seed used for comparable validation noise/timesteps.",
    )
    return parser
...
def add_model_config(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Resume from a complete checkpoint directory, 'latest', or a legacy weight file.",
    )
    parser.add_argument(
        "--initial_sparse_object_checkpoint_dir",
        default=None,
        type=str,
        help="Complete Sparse Object Interaction checkpoint directory used only for an exact weight warm-start.",
    )
    parser.add_argument(
        "--initial_sparse_object_checkpoint_sha256",
        default=None,
        type=str,
        help="Expected SHA256 of the warm-start trainable_model.safetensors payload.",
    )
    parser.add_argument(
        "--initial_sparse_object_checkpoint_step",
        default=None,
        type=int,
        help="Expected global_step recorded by the complete warm-start checkpoint.",
    )
    return parser

def add_training_config(parser: argparse.ArgumentParser):
    parser.add_argument("--train_batch_size", type=int, default=1, help="True per-rank DataLoader microbatch size.")
    parser.add_argument("--learning_rate", type=float, default=1e-4, help="Learning rate.")
    parser.add_argument(
        "--lora_lr_scale",
        type=float,
        default=1.0,
        help="LoRA learning-rate multiplier relative to --learning_rate.",
    )
    parser.add_argument(
        "--sparse_object_erase_gate_lr_multiplier",
        type=float,
        default=1.0,
        help="LR multiplier for newly initialized erase_then_write_v1 gate parameters.",
    )
    parser.add_argument("--num_epochs", type=int, default=1, help="Number of epochs.")
    parser.add_argument("--max_train_steps", type=int, default=None, help="Maximum number of optimizer steps across all epochs.")
    parser.add_argument("--seed", type=int, default=42, help="Global and deterministic data-order seed.")
    parser.add_argument(
        "--train_sampler_weights",
        type=str,
        default=None,
        help="Frozen per-row probabilities aligned exactly to the training override order.",
    )
    parser.add_argument(
        "--train_sampler_receipt",
        type=str,
        default=None,
        help="Frozen source/scene/group hierarchical sampler contract.",
    )
