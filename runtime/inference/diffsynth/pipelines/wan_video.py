...
class WanVideoPipeline(BasePipeline):
    ...
    def from_pretrained(
        ...
        enable_friction_controlnet: bool = False,
        enable_restitution_controlnet: bool = False,
        enable_force_controlnet: bool = False,
        force_condition_mode: str = None,
        enable_global_physics_cross_attn: bool = False,
        enable_oracle_slot_adapter: bool = False,
        enable_object_time_graph_adapter: bool = False,
        enable_sparse_object_interaction_adapter: bool = False,
        enable_phyparam_control_dit: bool = False,
        phyparam_num_control_blocks: int = None,
        phyparam_harmonic_bands: int = 8,
        phyparam_default_mass_normalized: float = 0.5,
        phyparam_max_objects: int = 8,
        phyparam_dino_feature_dim: int = 4096,
        phyparam_feature_tap_blocks: tuple[int, ...] = None,
        phyparam_enable_feature_supervision: bool = True,
        phyparam_dino_target_grid: tuple[int, int] = (14, 14),
        phyparam_temporal_min_norm: float = 1e-6,
        sparse_object_adapter_architecture: str = "v2",
        sparse_object_injection_blocks: tuple[int, ...] = (
        ...
        if isinstance(dit, list):
            ...
            pipe.dit = dit
        for candidate in (pipe.dit, pipe.dit2):
            if candidate is not None and enable_friction_controlnet:
                candidate.enable_friction_controlnet()
            if candidate is not None and enable_restitution_controlnet:
                candidate.enable_restitution_controlnet()
            if candidate is not None and enable_force_controlnet:
                candidate.enable_force_controlnet()
            if candidate is not None and force_condition_mode is not None:
                candidate.enable_force_conditioning(force_condition_mode)
            if candidate is not None and enable_global_physics_cross_attn:
                candidate.enable_global_physics_cross_attn()
            if candidate is not None and enable_oracle_slot_adapter:
                candidate.enable_oracle_slot_adapter()
            if candidate is not None and enable_object_time_graph_adapter:
                candidate.enable_object_time_graph_adapter()
            if candidate is not None and enable_sparse_object_interaction_adapter:
                candidate.enable_sparse_object_interaction_adapter(
                    architecture=sparse_object_adapter_architecture,
                    injection_blocks=sparse_object_injection_blocks,
                )
            if candidate is not None and enable_phyparam_control_dit:
                candidate.enable_phyparam_control_dit(
                    num_control_blocks=phyparam_num_control_blocks,
                    harmonic_bands=phyparam_harmonic_bands,
                    default_mass_normalized=phyparam_default_mass_normalized,
                    max_objects=phyparam_max_objects,
                    dino_feature_dim=phyparam_dino_feature_dim,
                    feature_tap_blocks=phyparam_feature_tap_blocks,
                    enable_feature_supervision=phyparam_enable_feature_supervision,
                    dino_target_grid=phyparam_dino_target_grid,
                    temporal_min_norm=phyparam_temporal_min_norm,
                )
    ...
    def __call__(
        ...
        friction_control_latents: torch.Tensor = None,
        restitution_control_latents: torch.Tensor = None,
        force_control_latents: torch.Tensor = None,
        oracle_slot_condition: dict = None,
        object_time_graph_condition: dict = None,
        sparse_object_condition: dict = None,
        ...
        inputs_shared = {
            ...
            "friction_control_latents": friction_control_latents,
            "restitution_control_latents": restitution_control_latents,
            "force_control_latents": force_control_latents,
            "oracle_slot_condition": oracle_slot_condition,
            "object_time_graph_condition": object_time_graph_condition,
            "sparse_object_condition": sparse_object_condition,
...
def model_fn_wan_video(
    ...
    friction_control_latents: Optional[torch.Tensor] = None,
    restitution_control_latents: Optional[torch.Tensor] = None,
    force_control_latents: Optional[torch.Tensor] = None,
    force_condition_tensor: Optional[torch.Tensor] = None,
    global_physics_tokens: Optional[list] = None,
    oracle_slot_condition: Optional[dict] = None,
    object_time_graph_condition: Optional[dict] = None,
    sparse_object_condition: Optional[dict] = None,
    disable_phyparam_dense_residual: bool = False,
    ...
    **kwargs,
    ...
    if sliding_window_size is not None and sliding_window_stride is not None:
        model_kwargs = dict(
            ...
            restitution_control_latents=restitution_control_latents,
            force_control_latents=force_control_latents,
            force_condition_tensor=force_condition_tensor,
            global_physics_tokens=global_physics_tokens,
            oracle_slot_condition=oracle_slot_condition,
            object_time_graph_condition=object_time_graph_condition,
            sparse_object_condition=sparse_object_condition,
        )
    ...
    force_condition_state = None
    # The two force interfaces use the clean Wan patch embedding plus either an
    # algebraically equivalent concat slice or a separately tokenised condition.
    if force_condition_tensor is not None:
        if (
            hasattr(dit, "wantodance_enable_global")
            and dit.wantodance_enable_global
            and int(wantodance_fps + 0.5) != 30
        ):
            raise ValueError("force conditioning is not implemented for WanToDance global patching")
        x, force_condition_state = dit.patchify_with_force(x, force_condition_tensor)
    elif hasattr(dit, "wantodance_enable_global") and dit.wantodance_enable_global and int(wantodance_fps + 0.5) != 30:
        x = dit.patchify(x, control_camera_latents_input, enable_wantodance_global=True)
    else:
        x = dit.patchify(x, control_camera_latents_input)

    # g/mu/e global physics tokens: independent of the F patch-embedding path
    # above, so this is safe to prepare regardless of which branch just ran.
    # Returns None only when physics conditioning is disabled for this model
    # (constant across ranks/steps); a zero-token sample still gets a real,
    # fully-masked state so every rank invokes the same modules every step.
    global_physics_state = dit.prepare_global_physics_tokens(
        global_physics_tokens,
        batch=x.shape[0],
        device=x.device,
        dtype=x.dtype,
    )
    ...
    # Reuse the existing provenance-aware object condition file verbatim. The
    # PhyParam branch and Mainline-2 adapter are intentionally mutually
    # exclusive, so one key does not activate two architectures in one run.
    phyparam_control_state = None
    if getattr(dit, "phyparam_control_enabled", False):
        phyparam_control_state = dit.prepare_phyparam_control(
            x,
            context,
    ...
    friction_control_state = dit.prepare_friction_controlnet(
        friction_control_latents,
        f,
        h,
        w,
        x.shape[0],
    )
    restitution_control_state = dit.prepare_restitution_controlnet(
        restitution_control_latents,
        f,
        h,
        w,
        x.shape[0],
    )
    force_control_state = dit.prepare_force_controlnet(
        force_control_latents,
        f,
        h,
        w,
        x.shape[0],
    ...
    # blocks
    if getattr(dit, "oracle_slot_enabled", False):
        dit.reset_oracle_slot_impact_logits()
    if getattr(dit, "object_time_graph_enabled", False):
        dit.reset_object_time_graph_outputs()
    if getattr(dit, "sparse_object_enabled", False):
        dit.reset_sparse_object_outputs()
    if use_unified_sequence_parallel:
        if dist.is_initialized() and dist.get_world_size() > 1:
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if vap is not None and block_id in vap.mot_layers_mapping:
                ...
                oracle_slot_condition is not None
                and getattr(dit, "oracle_slot_enabled", False)
                and str(block_id) in dit.oracle_slot_adapter.input_projections
            ):
                x = gradient_checkpoint_forward(
                    block.forward_self_attention,
                    use_gradient_checkpointing,
                    use_gradient_checkpointing_offload,
                    x, t_mod, freqs,
                )
                x = dit.apply_oracle_slot_adapter(
                    block_id,
                    x,
                    oracle_slot_condition,
                    timestep,
                    f,
                    h,
                    w,
                ...
                x = gradient_checkpoint_forward(
                    block.forward_cross_attention_ffn,
                    use_gradient_checkpointing,
                    use_gradient_checkpointing_offload,
                    x, context, t_mod,
                )
                ...
                and getattr(dit, "sparse_object_enabled", False)
                and str(block_id) in dit.sparse_object_adapter.groups
            ):
                x = gradient_checkpoint_forward(
                    block.forward_self_attention,
                    use_gradient_checkpointing,
                ...
                x = dit.apply_sparse_object_adapter(
                    block_id,
                    x,
                    sparse_object_condition,
                    timestep,
                ...
                and getattr(dit, "object_time_graph_enabled", False)
                and str(block_id) in dit.object_time_graph_adapter.groups
            ):
                x = gradient_checkpoint_forward(
                    block.forward_self_attention,
                    use_gradient_checkpointing,
                ...
                x = dit.apply_object_time_graph_adapter(
                    block_id,
                    x,
            ...
            phyparam_control_state, phyparam_residual = (
                dit.apply_phyparam_control_block(
                    block_id,
                    phyparam_control_state,
                    t_mod,
                    use_gradient_checkpointing,
                    use_gradient_checkpointing_offload,
                )
            )
            if phyparam_residual is not None:
                if not disable_phyparam_dense_residual:
                    x = x + phyparam_residual.to(dtype=x.dtype)

            x = dit.apply_force_condition_attention(
                block_id,
                x,
                force_condition_state,
                f,
                h,
                w,
            )
            x = dit.apply_global_physics_cross_attention(
                block_id,
                x,
                global_physics_state,
            )
            friction_control_state, friction_residual = dit.apply_friction_controlnet_block(
                block_id,
                friction_control_state,
                context,
                t_mod,
                freqs,
                f,
            ...
            if friction_residual is not None:
                x = x + friction_residual.to(dtype=x.dtype)
            restitution_control_state, restitution_residual = dit.apply_restitution_controlnet_block(
                block_id,
                restitution_control_state,
                context,
                t_mod,
                freqs,
                f,
            ...
            if restitution_residual is not None:
                x = x + restitution_residual.to(dtype=x.dtype)
            force_control_state, force_residual = dit.apply_force_controlnet_block(
                block_id,
                force_control_state,
                context,
                t_mod,
            ...
            if force_residual is not None:
                x = x + force_residual.to(dtype=x.dtype)
