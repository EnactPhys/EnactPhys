...
class ImageCropAndResize(DataProcessingOperator):
    ...
    def crop_and_resize(self, image, target_height, target_width):
        ...
        scale_fn = max if self.resize_mode == "crop" else min
        scale = scale_fn(target_width / width, target_height / height)
        ...
        if self.resize_mode == "crop":
            ...
            if delta_width < 0 or delta_height < 0:
                raise ValueError(
                    "pad resize unexpectedly exceeded target: "
                    f"{image.size} vs {(target_width, target_height)}"
                )
            left = delta_width // 2
            ...
            image = torchvision.transforms.functional.pad(
                image, (left, top, right, bottom), fill=0
            )
        return image
